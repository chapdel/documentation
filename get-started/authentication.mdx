...
1. **Test API Keys**: Used for development and testing. These keys contains `test_`.
2. **Live API Keys**: Used for production.
...
curl https://api.notchpay.co/payments \
  -H "Authorization: pk_test_123456789abcdef" \
...
```javascript
/ JavaScript example
const notchpay = new NotchPay('pk_test_123456789abcdef');
...
$headers = [
    'Authorization: pk_test_123456789abcdef',
...
1. **Never share your Private keys** in publicly accessible areas such as GitHub, client-side code, or social media.
