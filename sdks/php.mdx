...
<Frame>
  <img src="/images/php-sdk-flow.png" alt="PHP SDK Integration Flow" />
</Frame>

The diagram above illustrates the typical integration flow when using the Notch Pay PHP SDK:

1. **Initialize the SDK** - Set up the SDK with your API keys
2. **Create a Payment** - Generate a payment request with customer and transaction details
3. **Process the Payment** - Redirect the customer to complete the payment or process it directly
4. **Handle Webhooks** - Receive and process payment notifications
5. **Verify the Payment** - Confirm the payment status and fulfill the order
