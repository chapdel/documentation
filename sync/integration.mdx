...
4. Managing payouts to connected accounts
...
For direct charges where the payment is made directly to a connected account:

```javascript
// Create a direct charge
fetch('https://api.notchpay.co/sync/charges', {
  method: 'POST',
  headers: headers,
  body: JSON.stringify({
    amount: 10000,
    ...
    description: 'Payment for Service #456',
    reference: 'service_456',
    account: accountId, // The connected account ID
    application_fee_percent: 10 // Your platform fee (10%)
  })
})
.then(response => response.json())
.then(data => {
...
.catch(error => console.error('Error creating direct charge:', error));
```

### Managing Payouts

To create a payout to a connected account:

```javascript
// Create a payout
fetch('https://api.notchpay.co/sync/payouts', {
  method: 'POST',
  headers: headers,
  body: JSON.stringify({
    amount: 50000,
...
.then(data => {
  ...
  // Store the payout ID for future reference
})
...
Webhooks allow you to receive real-time notifications about events related to your connected accounts, payments, and payouts.
...
2. **Transaction Visibility**: View detailed transaction history, including payments, fees, and payouts
...
6. **Payment Settings**: Configure their payment preferences and payout schedules
...
3. **Transparent Payouts**: Provide clear information about payout schedules and amounts
...
- [Account Management](/docs/sync/account-management) - Learn how to manage connected accounts
- [Sync API Reference](/api/sync) - Detailed API documentation for Sync
- [Webhooks](/docs/webhooks) - Set up webhooks for real-time notifications
- [Testing](/docs/testing) - Test your Sync integration in the sandbox environment