...
In addition to [Mobile Money](/docs/accept-payments/mobile-money), Notch Pay supports various other payment methods to help you reach more customers across Africa.
...
Notch Pay also supports cryptocurrency payments, allowing customers to pay with digital currencies like Bitcoin, Ethereum, and others.

### How Cryptocurrency Payments Work

1. **Initialize Payment**: Create a payment with customer and transaction details
2. **Wallet Address Generation**: Notch Pay generates a unique wallet address for the payment
3. **Customer Transfer**: The customer transfers the cryptocurrency to the provided wallet address
4. **Confirmation**: Notch Pay confirms the payment status and notifies your application

### Supported Cryptocurrencies

Notch Pay supports various cryptocurrencies:

- Bitcoin (BTC)
- Ethereum (ETH)
- USDT
- Other cryptocurrencies depending on availability
...
.then(response => response.json())
.then(data => {
  // Redirect to checkout where cryptocurrency will be an option
  window.location.href = data.authorization_url;
})
```
