...
Notch Pay's API allows you to build a custom payment experience directly into your application. With our RESTful API, you have complete control over the payment flow while still benefiting from Notch Pay's secure payment processing.
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
  ...
  <div className="border rounded-lg p-6 hover:shadow-md transition-shadow">
    ...
    <ul className="list-disc pl-5">
      <li><a href="/docs/accept-payments/collect" className="text-blue-600 hover:underline">Collect</a>: For a quick integration with minimal coding</li>
      <li><a href="/docs/accept-payments/payment-links" className="text-blue-600 hover:underline">Payment Links</a>: For sharing payment links without coding</li>
      <li><a href="/docs/accept-payments/invoices" className="text-blue-600 hover:underline">Invoices</a>: For creating and managing invoices</li>
    </ul>
  </div>
</div>
...
```javascript
{
  "channel": "wallet.assoh", // or other wallet providers
  "data": {
    "phone": "+237680000000" // Customer's wallet phone number
  }
}
```
...
2. Use [test payment methods](/docs/testing/test-cards) to simulate different scenarios
...
2. **Implement Idempotency**: Use unique references for each payment to prevent duplicate charges
3. **Validate Input**: Validate all input parameters before sending them to the API
4. **Handle Errors Gracefully**: Implement proper error handling for a better user experience
5. **Use Webhooks**: Set up webhooks for reliable payment notifications
6. **Log Transactions**: Keep detailed logs of all transactions for troubleshooting
7. **Implement Retry Logic**: Add retry logic for API calls that may fail due to network issues
8. **Utilize Metadata**: Use metadata to link payments to your internal systems

## API Reference

For detailed API documentation, see the [Payments API Reference](/api/payments).
