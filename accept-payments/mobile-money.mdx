...
The simplest way to accept Mobile Money payments is through [Notch Pay Collect](/docs/accept-payments/collect). When creating a payment, the customer will be presented with Mobile Money as a payment option.
...
For a complete list of supported channels, use the [Channels API](/api/resources#list-payment-channels).
...
|------------|-------------|----------|
| `invalid_phone` | The phone number format is invalid | Check the phone number format |
| `insufficient_funds` | The customer has insufficient funds | Ask the customer to top up their account |
| `user_cancelled` | The customer cancelled the payment | Inform the customer and allow them to try again |
| `timeout` | The payment request timed out | Ask the customer to try again |
...
For more information on testing, see the [Testing Guide](/docs/testing).
